...
**这是 mracry 的 Sub2API 魔改版，不是 tosky / ranxi2001 原版。**

基于 [ranxi2001/sub2api](https://github.com/ranxi2001/sub2api) / [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) 魔改。  
由 [mracry](https://github.com/mracry) 维护，**不向原仓库提 PR，也不提供原作者的社区、文档站或发行通道。**

**本魔改无偿分享。** 用得上的话，请给仓库点个 Star：  
→ [https://github.com/mracry/sub2api](https://github.com/mracry/sub2api)

[本仓库](https://github.com/mracry/sub2api) · [魔改分支](https://github.com/mracry/sub2api/tree/feat/codex-harvest-flow-admin) · [门票钉住原理](CODEX门票钉住原理.md)
...
无偿分享的魔改版，不是官方发行版。没有微信群、没有二维码、没有 tosky 文档、没有 ranxi Release。觉得有用就给 [GitHub 点个 Star](https://github.com/mracry/sub2api)。

- 不要用 `ranxi2001/sub2api` 的安装脚本、镜像或更新通道来跑本魔改。
- 不要给原仓库提 PR。
- Codex 门票行为与原版不同：对话必须钉在打票出口和身份上。见 [CODEX门票钉住原理.md](CODEX门票钉住原理.md)。

## 魔改点

- **Codex 门票钉住**：打到 292 后，后续对话复用同一出口和同一会话身份；跟随 `x-codex-turn-state`；有活票就停探；对话期间暂停打票。
- 其余网关能力沿用上游，按需自用。
...
本魔改没有独立 Release。下面「脚本安装 / Docker 一键脚本」仍指向原仓库通道，装出来的是原版，不是本魔改。要跑门票钉住，请 clone 本仓库后源码编译。

```bash
git clone --branch feat/codex-harvest-flow-admin https://github.com/mracry/sub2api.git
```
...
**本魔改无偿分享。用得上的话，请给 [GitHub 点个 Star](https://github.com/mracry/sub2api)。**
